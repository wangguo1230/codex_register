...
function candidatePaths(): string[] {
    ...
    out.push(path.resolve(process.cwd(), "..", "pool_emails.txt"));
    out.push(path.resolve(process.cwd(), "..", "hotmail_inbox.txt"));
    // 去重保序
...
function historyPath(): string {
    // 跟随项目根（codex_register/ 的上一级），所有 worker 共享同一个 history
    return path.resolve(process.cwd(), "..", "hotmail_inbox.history.txt");
}
