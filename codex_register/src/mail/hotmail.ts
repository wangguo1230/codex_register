...
import {buildHotmailCredential} from "../email-reg/mailbox-credential.js";
...
function resolveTokensFile() {
    ...
    const candidates = [
        ...
        path.resolve(process.cwd(), "..", "pool_emails.txt"),
        path.resolve(process.cwd(), "..", "hotmail_inbox.txt"),
    ];
...
export function createHotmailProvider() {
    return {
        ...
        async getMailboxCredential(email) {
            ...
            return buildHotmailCredential(email, account);
        },
        async getEmailVerificationCode(email, options) {
            ...
            for (let attempt = 1; attempt <= HOTMAIL_POLL_ATTEMPTS; attempt += 1) {
                ...
                try {
                    const message = await getLatestVerificationMessage(email, account, minTimestampMs);
                    ...
                    // IMAP / REST 任一失败都不放弃整个 phase A —— 只是这一次 poll 没拿到，下一轮再试
